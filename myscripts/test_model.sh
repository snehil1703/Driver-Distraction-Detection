...
echo aprun python python/classify.py --pretrained_model=models/${TRAIN_CAT}/${TRAIN_CAT}_iter_${ITER}.caffemodel --model_def=models/${TRAIN_CAT}/deploy.prototxt --mean_file=data/ilsvrc12/imagenet_mean.npy --gpu ${TEST_FILE}-227.test.txt ${CATEGORIES}-out.txt
aprun python python/classify.py --pretrained_model=models/${TRAIN_CAT}/${TRAIN_CAT}_iter_${ITER}.caffemodel --model_def=models/${TRAIN_CAT}/deploy.prototxt --mean_file=data/ilsvrc12/imagenet_mean.npy --gpu ${TEST_FILE}-227.test.txt ${CATEGORIES}-out.txt
